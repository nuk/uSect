...
public class USectDriverTest {
	...
	@Test
	public void respondToConnectWithPlayerId() {
		...
		assertThat(response.getResponseData()).hasSize(1).contains(MapEntry.entry("player.id", id.toString()));
	}

	@Test
	public void connectCallingDeviceWithPlayer() throws Exception {
		UUID id = UUID.randomUUID();
		settings.put("usect.player.id", id.toString());
	...
	@Test
	public void redirectCallToCorrespondingPlayer() {
	...
	@Test
	public void allowsTheMigrationOfSects() throws Exception {
		Sect s = setUpEnvironment().addSect(new Sect(), new Point());
		USectDriver driver = new USectDriver(settings, e);
		Call call = new Call();
		call.addParameter("sect", mapper.treeToValue(s.toJSON(), Map.class));
